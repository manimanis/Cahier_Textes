...
document.addEventListener('DOMContentLoaded', () => {
  ...
  loadConfig(yearParam).then(() => {
    ...
    startApp(); // fallback avec valeurs par défaut
  });
});
...
function startApp() {
  new Vue({
      ...
      allCells() {
        ...
        this.hours.forEach((hour, idx) => {
          ...
          this.days.forEach(day => {
            ...
            cells.push({ id: 's' + gridRow + '-' + col, type: 'session', row: gridRow, col: col, rowspan: rowspan, colspan: 1, classe: session.classe, startTime: session.startTime, endTime: session.endTime, groupe: session.groupe === 'Toute la classe' ? '' : session.groupe });
          });
        });
        return cells;
      }
    ...
    methods: {
      ...
      changeYear() {
        window.location.href = 'emploi_temps.html?year=' + encodeURIComponent(this.selectedYear);
      }
    }
  });
}